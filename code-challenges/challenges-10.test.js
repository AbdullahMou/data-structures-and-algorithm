...
const totalSum = (input) => {
    // Solution code here...
    let finalResult = input.reduce(function(x, i) {
        let innerArr = i.reduce(function(y, j) {
            y += j;
            return y;
        }, 0);
        x += innerArr;
...
const divisibleByFiveTwoToThePower = (input) => {
    // Solution code here...
    let divisableNum = input.map((val, i) => {
        return input[i].filter(ele => {
            return typeof ele === 'number' && ele % 5 === 0;
        });
    });

    return divisableNum.map((val, i) => {
        return divisableNum[i].map(ele => {
...
describe('Testing challenge 4', () => {
    ...
    test('It should return an empty array if the eles are not numbers', () => {
        expect(divisibleByFiveTwoToThePower([
            ['one', 'two', 'five'],
            ['5', '10', '15'],
            [5]
